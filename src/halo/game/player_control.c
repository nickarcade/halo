...
#ifndef input_get_mouse_state
#define input_get_mouse_state FUN_000cf690
#endif
...
void get_local_player_input_blob(void *action, short local_player_index, real delta_time)
{
  ...
  } else if (input_get_mouse_state() && is_current_controller) {
    char *mouse;
...

  {
    real move_mag_sq;
    real inv_mag;
    move_mag_sq = input->field_0x00 * input->field_0x00 + input->field_0x04 * input->field_0x04;
    if (move_mag_sq > 1.0f) {
      inv_mag = 1.0f / sqrtf(move_mag_sq);
...
tail:
  player_control_action_test_check_reset_input_blob((char *)action);
