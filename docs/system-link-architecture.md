...
The server's in-game player quit routine now uses a fixed-size `memcpy` in the VC71 build, matching the 2276 inline `rep movsd` copy of the 0x20-byte player. Its VC71 mnemonic and operand match rose from 90.4% to 100%; relocation-aware comparison rose from 107/173 to 148/164 aligned bytes, with relocation targets still unverified. The tracked [`check_system_link_server_quit_ingame.py`](../tools/equivalence/check_system_link_server_quit_ingame.py) compares the full 0x24-byte message payload passed by 2276 and the built C on a matching-player path. Its synthetic snapshot reaches 68.4% of the original body; downstream message creation and broadcast remain stubbed.

`network_game_server_send_message_to_machine` now returns a byte-sized `bool`, matching PAL's `boolean` and the 2276 AL return. Its VC71 mnemonic match rose from 87.7% to 100%, and the relocation-aware comparison is structurally exact for 30/30 instructions and 53/53 compared bytes. Linked relocation bytes remain unverified. Synthetic differential checks pass for no connection, write success, and write failure; the connected success case reaches 91.8% of the original body. The connection writer is stubbed, so network delivery remains unverified.

`countdown_timer_update` now places its last-update assignment after the elapsed-time branch, as PAL does. The resulting VC71 body matches the 2276 instruction structure (23/23 instructions, 44/44 compared bytes), raising mnemonic match from 90.5% to 100% with no server-source regression. Synthetic differential checks pass for subtraction, zeroing, no elapsed time, and the signed wrap boundary; `system_milliseconds` is stubbed. Linked call bytes and runtime timer behavior remain unverified.

`countdown_timer_increment` now uses explicit 32-bit wrapping arithmetic for its elapsed-time subtraction and adjustment addition, and PAL's stored minimum expression for the clamp. Its VC71 mnemonic and operand match rose from 91.1% to 100% without another server-source regression. The candidate and 2276 body are both 144 bytes; the relocation-aware audit aligns 108/124 bytes (provisional 87.1–100%), with unresolved relocation targets preventing a literal-byte verdict. Six synthetic differential paths passed, including adjustment overflow and elapsed-time wrap; the final wrapping casts were rerun on those two paths and the normal path. `system_milliseconds` remains stubbed, so runtime timer behavior is unverified.

`countdown_timer_decrement` now uses explicit 32-bit wrapping arithmetic for the tick and adjustment subtractions, PAL's comparison order, and its shared final assertion. VC71 mnemonic and operand match rose from 91.6% to 100%, with no server-source regression. Candidate and 2276 bodies are both 141 bytes; the relocation-aware audit improved from 97/122 to 105/121 aligned bytes (provisional 86.8–100%), with relocation targets unresolved. Five synthetic differential paths pass, covering ordinary subtraction, the zero floor, elapsed time, no clock advance, and elapsed-time wrap. `system_milliseconds` is stubbed, and live timer behavior is unverified.
...
`network_game_client_handle_game_update` now follows PAL's missed-sequence branch before its matching-sequence clock and seed checks. Provisional relocation-aware aligned bytes improve from 225/361 (62.3-67.9%) to 239/329 (72.6-79.9%); literal byte identity is unverified. VC71 mnemonic match rises from 83.6% to 92.9% and operand match from 73.0% to 81.9%, with no regression among the 60 measured client-manager functions. The lift pipeline passes build, ABI, hazard, and buffer gates. The unconstrained 100-seed differential has 97 invalid-input oracle errors, so the tracked [`check_system_link_client_game_update.py`](../tools/equivalence/check_system_link_client_game_update.py) runner uses five valid states instead: matching and missed sequences, local clock and seed differences, and player-count expansion. All five agree with 2276 on return and client/message state, including the next sequence, timestamp, and expanded count; individual oracle coverage ranges from 38.0% to 62.5%. Buffer copying and downstream update handling are stubbed, and live game-update behavior remains unverified.

PAL's `network_game_client_server_has_started_game` tests whether the signed next-update number is positive. The 2276 instructions at `0x12580b`–`0x125817` load the number, compare zero against it, then use carry subtraction and negation, returning true for any nonzero 32-bit value. The active C retains `!= 0`; a PAL-style `> 0` experiment did not improve the provisional 36/53 aligned-byte count and would change behavior for values with the high bit set.

The 2276 client acceptance path now follows PAL's nested valid-index and message-result branches while retaining the existing state changes and calls. Its provisional aligned-byte match improved from 192/256 to 206/232; VC71 mnemonic match rose from 90.2% to 100%, with no regression in the 60 measured functions in the translation unit. Six controlled 2276-versus-C cases cover valid indices, invalid indices, message-creation failure, and write failure, with 26.1–74.3% oracle instruction coverage and matching client state. Helpers are stubbed in these cases, so packet contents, helper arguments, live joining, and literal byte identity remain unverified.

PAL's `network_game_server_add_new_client` keeps the valid-address path outermost and nests the remote-connection gate inside it. Matching that branch layout in 2276 C improved provisional aligned-byte match from 196/338 to 227/305 and VC71 mnemonic match from 84.1% to 97.0%, without a regression among the 84 measured functions in the server-manager translation unit. Eight synthetic 2276-versus-C cases pass for closed/full games, invalid addresses, remote rejection, accepted connections, failed acceptance, and a later free slot. Their helpers are stubbed and each case uses one seed; literal byte identity and live connection behavior remain unverified.

PAL's `network_game_server_graceful_shutdown` handles both message-creation cases through a shared final send check. Restoring that structure in 2276 C raised the provisional aligned-byte count from 123/203 to 155/187 and VC71 mnemonic and operand match from 88.6%/85.0% to 100%/100%. The raw comparator still classifies eight relocation targets as unresolved, so its verdict is not comparable for literal identity. Six synthetic 2276-versus-C cases pass across pregame, postgame, creation failure, send failure, and an inactive state, with 16.0–39.0% oracle instruction coverage. Helpers are stubbed; packet arguments and live shutdown behavior remain unverified.

PAL's nested result handling improves the 2276 `network_game_client_handle_message_server_game_settings_update` C: provisional aligned-byte match rises from 177/252 to 186/262, and VC71 mnemonic match from 89.9% to 99.5%, with no regression among 60 measured functions in the client-manager translation unit. The 2276 instructions store packet type and version as 32-bit locals, so those remain `int` despite PAL's `short` declarations. Five synthetic 2276-versus-C cases pass for bad source, wrong state, decode failure, and settings-update failure/success, with 21.3–56.4% oracle instruction coverage. The equivalence harness handles a same-object address helper asymmetrically; helper-call trace comparison is disabled for these cases, and packet contents, helper arguments, live joining, and literal byte identity remain unverified.

PAL and the 2276 instructions reset the client's incoming-message read capacity to `0x800` after handling each packet, then test the handler result before another read. Moving the C reset to that location raises provisional aligned bytes from 69/116 to 98/116 and VC71 mnemonic/operand match from 90.5%/83.3% to 97.6%/97.6%, without a regression among 60 measured client-manager functions. Four synthetic 2276-versus-C paths pass: empty read, handler failure, one success then empty read, and two packets with the second handler failing. The focused probe compares each helper call, packet bytes, source address, and the `0x800` capacity on every read. The KB's commented `@<esi>` is already parsed as a register argument and passes its immutable ABI baseline check; the probe supplies the register parameter under its parsed name `p0`. Network reads and message handling are stubbed, so live delivery and literal linked-byte identity remain unverified.
